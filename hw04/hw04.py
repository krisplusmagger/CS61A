...
def balanced(m):
    ...
    if is_planet(m):
        return True
    return balanced(end(left(m))) and balanced(end(right(m))) and total_weight(
        end(left(m))) * length(left(m)) == total_weight(
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    new_branch = []        
    for b in branches(t):
        new_branch += [replace_loki_at_leaf(b, lokis_replacement)]
